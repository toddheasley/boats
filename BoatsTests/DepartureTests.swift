...
class DepartureTests: XCTestCase {
    func testJSONEncoding() {
        guard let mockJSON = mockJSON, let departure = Departure(JSON: mockJSON), let JSON = departure.JSON as? [String: AnyObject] else {
            XCTFail()
            return
        }
    ...
    func testJSONDecoding() {
        guard let mockJSON = mockJSON, let departure = Departure(JSON: mockJSON) else {
            XCTFail()
            return
        }
