...
class RouteTests: XCTestCase {
    func testSchedule() {
        guard let mockJSON = mockJSON, let route = Route(JSON: mockJSON) else {
            XCTFail()
            return
        }
        XCTAssertNotNil(route.schedule(date: Date(JSON: "2016-05-01")!))
        XCTAssertTrue(route.schedule(date: Date(JSON: "2016-04-01")!) == nil)
        XCTAssertTrue(route.schedule(date: Date(JSON: "2016-10-15")!) == nil)
    }
    
    func testJSONEncoding() {
        guard let mockJSON = mockJSON, let route = Route(JSON: mockJSON), let JSON = route.JSON as? [String: AnyObject] else {
            XCTFail()
            return
        }
